...
def ingest_emotions(records: List[Dict]) -> None:
    ...
    # Fresh collection each run by upserting with deterministic IDs
    ...
    for i, rec in enumerate(records):
        ...
        ids.append(f"emotion_{i}_{rec.get('word','').lower().replace(' ','_')}")
        metas.append(meta)
...
def ingest_task_doc(path: str) -> None:
    ...
    try:
        ...
        # Fallback if there are any lingering DB state issues
    ...
    for i, sec in enumerate(sections):
        ...
        docs.append(doc_text)
        ids.append(f"task_sec_{i}_{sec['heading'][:20].replace(' ', '_')}")
        metas.append(meta)
